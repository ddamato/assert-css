...
function selectorMatcher(property, value) {
  const declarationAst = cssTree.find(this, (node) => node?.type === "Declaration" && node?.property === property);
  ...
  if (results.property && typeof value !== "undefined") {
    ...
    if (results.value) {
      ...
      results.value = typeof value === "function" ? value(valueStr) : valueStr === String(value);
    }
  }
  return results;
}
function atRuleMatcher(value) {
  const atRuleAst = cssTree.find(this, (node) => {
    if (node?.type !== "Atrule" || !node?.prelude?.value)
      return;
    return typeof value === "function" ? value(node.prelude.value) : node.prelude.value.includes(value);
  });
  return { atRule: Boolean(atRuleAst) };
}
function selector(str) {
  const ruleAst = cssTree.find(this, (node) => node.type === "Rule" && node.prelude.value === str);
  return {
    exists() {
      if (!Boolean(ruleAst))
        throw new Error(`Selector ${str} does not exist`);
    },
    includes(...args) {
    ...
    not: {
      exists() {
        if (Boolean(ruleAst))
          throw new Error(`Selector ${str} does exist`);
      },
      includes(...args) {
        const { property, value } = selectorMatcher.call(ruleAst, ...args);
...
function atRule(str) {
  const ruleAst = cssTree.find(this, (node) => node.type === "Atrule" && `@${node.name}` === str);
  return {
    exists() {
      if (!Boolean(ruleAst))
        throw new Error(`At-rule ${str} does not exist`);
    },
    includes(...args) {
      const { atRule: atRule2 } = atRuleMatcher.call(ruleAst, ...args);
      const result = typeof args[0] === "function" ? `from Function` : args[0];
      if (!atRule2)
        throw new Error(`At-rule ${str} does not match ${result}`);
    },
    selector: selector.bind(ruleAst),
    not: {
      exists() {
        if (Boolean(ruleAst))
          throw new Error(`At-rule ${str} does exist`);
      },
      includes(...args) {
        const { atRule: atRule2 } = atRuleMatcher.call(ruleAst, ...args);
        if (atRule2)
          throw new Error(`At-rule ${str} does match ${args[0]}`);
      }
    }
  };
}
